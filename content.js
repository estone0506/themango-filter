...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    // [자동 삭제] "마켓삭제시작(검색결과모든상품)" 연동 실행
    if (request.action === "TRIGGER_DELETE") {
        executeMarketDeleteOnPage();
...
function executeMarketDeleteOnPage() {
    // 페이지의 컨텍스트에서 코드를 실행하기 위해 스크립트 주입
    const script = document.createElement('script');
    script.textContent = `
        (function() {
            // 1. 브라우저 확인창(confirm) 자동 '확인' 처리
            window.confirm = function() { return true; };
            
            console.log('더망고 확장앱: 삭제 프로세스 시작');

            // 2. 실제 "마켓삭제시작 (검색결과모든상품)" 버튼 찾기
            const deleteButtons = Array.from(document.querySelectorAll('a.defbtn_med.dtype2'));
